...
#define PAGE_SIZE       2*1024  //1KB   
#define STACK_SIZE      64    //2.25KB
#define BLOCKSTACK_SIZE 32      //1.25KB
#define CALLSTACK_SIZE  32      //0.5KB
#define BR_TABLE_SIZE   32    //1KB
...
/*note:We changed the static array into dynamic forms,in order to
be suitable for more compilers and more flexible sizes, but make sure
that the calloc is avaliable.*/
typedef struct Module {
    ...
    Block       *localBlockStart; ///the position of local block array
    ...
    uint32_t        csp;               // callstack pointer
    Frame       *callstack; // callstack
    uint32_t    *br_table; // br_table branch indexes
    uint32_t opCount;//the amount of opcodes that has been run
    //Block *runCB;//current running block
    uint32_t runIdx;//the index of current running funtion
    uint8_t mtaskEn;//if multitask is using
    uint8_t isSafe;
    int8_t isRun;
