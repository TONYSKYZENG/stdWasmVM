...
int8_t vapor_sendVack(NETSOCKET *netsock)
{
    ...
    TDPPACK s_tdp;
    TDPPACK *m_tdp=&s_tdp;
    uint8_t tBin[sizeof(TDPPACK)-sizeof(uint8_t *)];
    ...
    //next, send 
    netsocket_send(netsock,tBin,sizeof(TDPPACK)-sizeof(uint8_t *));
    return 1;
...
{
    TDPPACK *tpack;
    if(readSize<sizeof(TDPPACK)-sizeof(uint8_t *))
    {
        return 0;
    }
   tpack=(TDPPACK *)rawBin;
    if(readSize!=tpack->size+sizeof(TDPPACK)-sizeof(uint8_t *))
    {
           return 0;
    }
