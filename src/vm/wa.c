...
//to count how many blocks are in this module

uint32_t count_block(Module *m)
{   Block    *function;
    Block    *block;
...
void find_blocks(Module *m) {
    ...
    int       top = -1;
    uint8_t   opcode = 0x00;
    uint32_t blkAll,blkNow;
    blkAll=count_block(m);
    //make sure all blocks are in the same memory area
    blockArray=(Block *)calloc(1,sizeof(Block)*blkAll);
    m->localBlockStart=blockArray;
    blkNow=0;
  // printf("%d blocks in this module\r\n",count_block(m));
   //scanf("%d",&tt);
...
bool interpret(Module *m) {
    ...
    while (m->pc < m->byte_count) {
        ...
        if(m->mtaskEn==1)
		{	m->opCount++;
		if(m->opCount==VM_OPCYCLE)
			{
				m->opCount=0;
			//	printf("this is an opcycle!\r\n");
				if(temp_taskMr_refresh(m)==1)
                {
                    return true;
                }
                //break;
...
Module *load_module(uint8_t *bytes, uint32_t byte_count, Options options) {
    ...
    m->start_function = -1;
  m->stack=(StackValue *)calloc(1,STACK_SIZE*sizeof(StackValue));
  if(m->stack==0)
  	{
  ...
  m->callstack=(Frame *)calloc(1,CALLSTACK_SIZE*sizeof(Frame));
  if(m->callstack==0)
  	{
  ...
  m->br_table=(uint32_t *)calloc(1,BR_TABLE_SIZE*sizeof(uint32_t));
  if(m->br_table==0)
  	{
  ...
  m->stackSize=STACK_SIZE;
  m->callstackSize=CALLSTACK_SIZE;
  m->br_tableSize=BR_TABLE_SIZE;
    // Check the module
    pos = 0;
